module EnglishNepaliDateConverter
	...
	class DateConversion
		...
		def np_date_valid(date)
			...
			invalid << "day" if BsDate::BS_DATES[year.index(date[2])][date[1]] < date[0]
		...
		def nep_to_eng(*args)
			...
			unless date_error.empty?
				...
				self.week_day = self.class.get_day_of_week(day_of_week, 'en')
				self.month = self.class.get_month_name(month_number, 'en')
				self.month_number = month_number
				self.day_in_month = day_in_month
				self.day_of_week = day_of_week
				self.year = year
