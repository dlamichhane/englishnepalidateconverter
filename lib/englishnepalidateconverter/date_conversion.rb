...
module EnglishNepaliDateConverter
  class DateConversion
    ...
    def eng_to_nep(*args)
      ...
      if date_error.empty?
        formatted_date = "#{args[0]}-#{args[1]}-#{args[2]}"
        ...
        year = 2000
        month_number = np_start_month = 9
        day_in_month = 17

        starting_index_of_bs_date = 0
        day_of_week = 7 # Saturday as Day 7 and Sunday as Day 1 of the week

        while en_total_days != 0
          day_in_month += 1
          day_of_week += 1
          if day_in_month > BsDate::BS_DATES[starting_index_of_bs_date][np_start_month]
            (month_number += 1
             day_in_month = 1
             np_start_month += 1)
          end
          ...
          if np_start_month > 12
            (np_start_month = 1
             starting_index_of_bs_date += 1)
          end
          en_total_days -= 1
        end
        ...
        date_error.collect { |error| "Enter appropriate #{error}" }
      end
    end
    ...
    def np_date_valid(date)
      ...
      invalid << "day" if invalid.empty? && (date[0].zero? || BsDate::BS_DATES[year.index(date[2])][date[1]] < date[0])
      invalid
    end
    ...
    def nep_to_eng(*args)
      ...
      if date_error.empty?
        ...
        np_year_diff = args[2] - np_start_year - 1
        np_total_days = 0

        (0..np_year_diff).each do |idx|
          np_total_days += BsDate::BS_DATES[idx][1..-1].inject(:+)
        end
        ...
        day_in_month = en_start_day
        month_number = en_start_month
        year = en_start_year
